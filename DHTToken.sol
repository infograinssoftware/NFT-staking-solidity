...
contract DHTToken is ERC1155 {
    ...
    mapping(uint256 => dht) public alldht;
    ...
    constructor() public ERC1155("HeroCollection") {
        collectionName = "DHT Collection";
        collectionNameSymbol = "DHTToken";
    ...
    function mintHero(
        ...
        if (tokenNameExists[_name]) {
            uint256 i = 1;
            DHT memory hero = alldht[i];
            for (i = 1; i <= dhtCounter; i++) {
                dht = alldht[i];
                if (
                    keccak256(abi.encodePacked(dht.tokenName)) ==
            ...
            require(
                keccak256(abi.encodePacked(dht.tokenURI)) ==
                    keccak256(abi.encodePacked(_tokenURI))
            );
            bytes memory data;
            _mint(msg.sender, dhtCounter, 1, data);
        } else {
            dhtCounter++;
            ...
            DHT memory newDHT = DHT(dhtCounter, _name, _tokenURI);
            alldht[dhtCounter] = newDHT;
        }
